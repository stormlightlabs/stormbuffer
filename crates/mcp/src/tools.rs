...
pub fn call(
    paths: &core::StorePaths, write_policy: McpWritePolicy, operation: &str, mut arguments: JsonObject,
    cancelled: bool, embedder: Option<&dyn core::Embedder>, unavailable_reason: Option<core::SemanticFallbackReason>,
    ...
    let envelope = if write && !write_policy.allows(operation) {
        let message = match write_policy {
            McpWritePolicy::ReadOnly => "MCP write tools are disabled; restart with an explicit host write grant",
            McpWritePolicy::CandidateOnly => "MCP candidate-write mode permits only remember and update",
            McpWritePolicy::All => unreachable!("all supported MCP writes are permitted"),
        };
        core::invoke_envelope(operation, Err(core::InvokeFailure::new("permission_denied", message)))
    } else {
        ...
        core::invoke_envelope(
            operation,
            core::invoke_operation_with_semantic_status(paths, operation, &arguments, embedder, unavailable_reason),
        )
    };
    let encoded = serde_json::to_string(&envelope)
        .map_err(|_| RmcpError::internal_error("could not encode tool result", None))?;
    if encoded.len() > MAX_TOOL_ENVELOPE_BYTES {
        return Ok(error_result(operation, "response exceeds the bounded MCP tool output"));
    }
