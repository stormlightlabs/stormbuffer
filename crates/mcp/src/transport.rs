...
pub fn run_stdio_with_config(config: McpConfig) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let use_default_embedder = !cfg!(debug_assertions) || std::env::var_os("STORMBUFFER_TEST_MODE").is_none();
    let runtime = tokio::runtime::Builder::new_current_thread().enable_all().build()?;
    runtime.block_on(async move {
        let server = if cfg!(debug_assertions) && std::env::var_os("STORMBUFFER_TEST_EMBEDDER").is_some() {
