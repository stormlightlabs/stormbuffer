...
pub fn render(root: Command) -> Result<GeneratedArtifacts, Box<dyn Error>> {
    ...
    man_pages.insert(format!("{program}.1"), output);
    ...
    for subcommand in subcommands {
        ...
        let mut output = Vec::new();
        Man::new(subcommand.name(format!("{program}-{name}"))).render(&mut output)?;
        man_pages.insert(format!("{program}-{name}.1"), output);
    }
    ...
    for (shell, extension) in [
        ...
        generate(shell, &mut completion_command, &program, &mut output);
        completions.insert(format!("{program}.{extension}"), output);
    }
